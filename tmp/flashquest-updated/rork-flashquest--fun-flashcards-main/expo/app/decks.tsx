    ...
    <View style={[styles.container, { backgroundColor: theme.background }]}> 
      ...
      <SafeAreaView style={styles.safeArea} edges={['top', 'bottom']}>
        ...
        <View style={styles.sortRow}>
          <ArrowUpDown color={theme.textTertiary} size={14} />
          {(['newest', 'name', 'cards'] as const).map((option) => {
            const isActive = sortBy === option;
            const label = option === 'newest' ? 'Newest' : option === 'name' ? 'A–Z' : 'Most Cards';
            return (
              <TouchableOpacity
                key={option}
              ...
              >
                <Text style={[styles.sortChipText, { color: isActive ? '#fff' : theme.textSecondary }]}>
                  {label}
                </Text>
              </TouchableOpacity>
            );
