...
export function getLevelRankBandLabel(level: number): string {
  return getLevelRankBandInfo(level).label;
}

export function getLevelRankBandTitle(level: number): string {
  const rankInfo = getLevelRankBandInfo(level);
  return `${rankInfo.label} Rank`;
}

export function getLevelRankBandProgress(level: number): number {
  const rankInfo = getLevelRankBandInfo(level);
  if (rankInfo.maxLevel === null) {
    return 1;
  }

  const bandSpan = Math.max(rankInfo.maxLevel - rankInfo.minLevel, 1);
  return Math.min((level - rankInfo.minLevel) / bandSpan, 1);
}

export function getLevelRankBandDescription(level: number): string {
  const rankInfo = getLevelRankBandInfo(level);
  switch (rankInfo.band) {
    case 'foundation':
      return 'Learning the shape of your study rhythm.';
    case 'momentum':
      return 'Building consistency and speed across sessions.';
    case 'skilled':
      return 'Turning strong habits into dependable performance.';
    case 'advanced':
      return 'Refined recall, sharper instincts, stronger control.';
    case 'prestige':
      return 'A top-tier study identity with lasting presence.';
  }
}

export function getLevelRankBandByIndex(index: number): LevelRankBandInfo | null {
  return LEVEL_RANK_BANDS[index] ?? null;
}

export function getLevelRankBandCount(): number {
  return LEVEL_RANK_BANDS.length;
}

export function getLevelRankBandOrder(level: number): number {
  return LEVEL_RANK_BANDS.findIndex((item) => item.band === getLevelRankBandInfo(level).band);
}

export function getLevelRankBandLevels(rankInfo: LevelRankBandInfo): number[] {
  const maxLevel = rankInfo.maxLevel ?? 20;
  return Array.from({ length: maxLevel - rankInfo.minLevel + 1 }, (_, index) => rankInfo.minLevel + index);
}

export function getLevelRankBandSummary(level: number): string {
  const rankInfo = getLevelRankBandInfo(level);
  return `${rankInfo.label} · ${formatLevelRankRange(rankInfo)}`;
}

export function getLevelRankBandNext(level: number): LevelRankBandInfo | null {
  const currentIndex = LEVEL_RANK_BANDS.findIndex((item) => item.band === getLevelRankBandInfo(level).band);
  if (currentIndex < 0 || currentIndex >= LEVEL_RANK_BANDS.length - 1) {
    return null;
  }

  return LEVEL_RANK_BANDS[currentIndex + 1] ?? null;
}

export function getLevelRankBandPrevious(level: number): LevelRankBandInfo | null {
  const currentIndex = LEVEL_RANK_BANDS.findIndex((item) => item.band === getLevelRankBandInfo(level).band);
  if (currentIndex <= 0) {
    return null;
  }
...
export function isSameLevelRankBand(leftLevel: number, rightLevel: number): boolean {
  return getLevelRankBandInfo(leftLevel).band === getLevelRankBandInfo(rightLevel).band;
}

export function getLevelRankBandIndex(level: number): number {
  return LEVEL_RANK_BANDS.findIndex((item) => item.band === getLevelRankBandInfo(level).band);
}

export function getLevelRankBandLevelCount(level: number): number {
  const rankInfo = getLevelRankBandInfo(level);
  const maxLevel = rankInfo.maxLevel ?? 20;
  return maxLevel - rankInfo.minLevel + 1;
}

export function getLevelRankBandStep(level: number): number {
  const rankInfo = getLevelRankBandInfo(level);
  return level - rankInfo.minLevel + 1;
}

export function getLevelRankBandMaxLevel(level: number): number | null {
  return getLevelRankBandInfo(level).maxLevel;
}

export function getLevelRankBandMinLevel(level: number): number {
  return getLevelRankBandInfo(level).minLevel;
}

export function getLevelRankBandIsFinal(level: number): boolean {
  return getLevelRankBandInfo(level).maxLevel === null;
}

export function getLevelRankBandHeader(level: number): string {
  return getLevelRankBandSummary(level);
}

export function getLevelRankBandCaption(level: number): string {
  return getLevelRankBandDescription(level);
}

export function getLevelRankBandLevelsRemaining(level: number): number {
  const rankInfo = getLevelRankBandInfo(level);
  if (rankInfo.maxLevel === null) {
    return 0;
  }

  return Math.max(rankInfo.maxLevel - level, 0);
}

export function getLevelRankBandStart(level: number): number {
  return getLevelRankBandInfo(level).minLevel;
}

export function getLevelRankBandEnd(level: number): number | null {
  return getLevelRankBandInfo(level).maxLevel;
}

export function getLevelRankBandHasMultipleLevels(level: number): boolean {
  const rankInfo = getLevelRankBandInfo(level);
  return rankInfo.maxLevel === null || rankInfo.maxLevel > rankInfo.minLevel;
}

export function getLevelRankBandBadgeLabel(level: number): string {
  return getLevelRankBandInfo(level).label;
}

export function getLevelRankBandHeroLabel(level: number): string {
  return getLevelRankBandInfo(level).label;
}

export function getLevelRankBandModalLabel(level: number): string {
  return getLevelRankBandInfo(level).label;
}

export function getLevelRankBandCurrentLabel(level: number): string {
  return `${getLevelRankBandInfo(level).label} · Lv ${level}`;
}

export function getLevelRankBandMeta(level: number): string {
  return `${getLevelRankBandInfo(level).label} rank`;
}

export function getLevelRankBandRowLabel(level: number): string {
  return getLevelRankBandInfo(level).label;
}

export function getLevelRankBandTier(level: number): string {
  return getLevelRankBandInfo(level).label;
}

export function getLevelRankBandShortRange(level: number): string {
  return formatLevelRankRange(getLevelRankBandInfo(level));
}

export function getLevelRankBandLongLabel(level: number): string {
  return `${getLevelRankBandInfo(level).label} ${formatLevelRankRange(getLevelRankBandInfo(level))}`;
}

export function getLevelRankBandHeroSubtitle(level: number): string {
  return getLevelRankBandDescription(level);
}

export function getLevelRankBandModalSubtitle(level: number): string {
  return getLevelRankBandDescription(level);
}

export function getLevelRankBandHasPrestige(level: number): boolean {
  return getLevelRankBandInfo(level).band === 'prestige';
}

export function getLevelRankBandHasAdvanced(level: number): boolean {
  return getLevelRankBandInfo(level).band === 'advanced';
}

export function getLevelRankBandHasSkilled(level: number): boolean {
  return getLevelRankBandInfo(level).band === 'skilled';
}

export function getLevelRankBandHasMomentum(level: number): boolean {
  return getLevelRankBandInfo(level).band === 'momentum';
}

export function getLevelRankBandHasFoundation(level: number): boolean {
  return getLevelRankBandInfo(level).band === 'foundation';
}

export function getLevelRankBandRangeLabel(level: number): string {
  return formatLevelRankRange(getLevelRankBandInfo(level));
}

export function getLevelRankBandTitleLabel(level: number): string {
  return `${getLevelRankBandInfo(level).label} Rank`;
}

export function getLevelRankBandDisplay(level: number): string {
  return `${getLevelRankBandInfo(level).label} · ${formatLevelRankRange(getLevelRankBandInfo(level))}`;
}

export function getLevelRankBandHint(level: number): string {
  return getLevelRankBandDescription(level);
}

export function getLevelRankBandMarker(level: number): string {
  return getLevelRankBandInfo(level).label;
}

export function getLevelRankBandSectionLabel(level: number): string {
  return getLevelRankBandInfo(level).label;
}

export function getLevelRankBandSectionRange(level: number): string {
  return formatLevelRankRange(getLevelRankBandInfo(level));
}

export function getLevelRankBandSectionTitle(level: number): string {
  return `${getLevelRankBandInfo(level).label} Rank`;
}

export function getLevelRankBandSectionDescription(level: number): string {
  return getLevelRankBandDescription(level);
}

export function getLevelRankBandSectionMeta(level: number): string {
  return `${getLevelRankBandInfo(level).label} · ${formatLevelRankRange(getLevelRankBandInfo(level))}`;
}

export function getLevelRankBandSectionCurrent(level: number): string {
  return `${getLevelRankBandInfo(level).label} rank`;
}

export function getLevelRankBandSectionHint(level: number): string {
  return getLevelRankBandDescription(level);
}

export function getLevelRankBandPipCount(level: number): number {
  switch (getLevelRankBandInfo(level).band) {
    case 'foundation':
      return 1;
    case 'momentum':
      return 2;
